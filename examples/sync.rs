...
fn __pender(_context: *mut ()) {
    // Required by Embassy's executor system, even when not using async
    // Empty implementation since we're not using async features
}
...
fn main() -> ! {
    ...
    let mut flash = spi_memory::series25::Flash::init(spi_device).unwrap();
